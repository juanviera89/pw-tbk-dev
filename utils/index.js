...
//console.log(testDate('(8-13.7 15-25 * *)'))
module.exports = {
    all: (arr, fn = Boolean) => arr.every(fn),
    date102Format: (dateString) => {
        const dateObject = new Date(dateString || Date.now());
        return `${dateObject.getFullYear()}.${dateObject.toISOString().substr(5, 2)}.${dateObject.toISOString().substr(8, 2)}`
    },
