...
const list = async (req, res, next) => {
  try {
    ...
    const pagosRealizados = await pago
      .find(
        {
...
const startPayment = async (req, res, next) => {
  ...
  try {
    ...
    if (!["webpay", "oneclick"].includes(tipo))
      return res
        .status(400)
        .send({ message: "Payment type not supported", oc: null });
    const userAttr = req.userInfo;
    if (tipo == "oneclick") {
    ...
    for (const eq of equipos) {
      ...
      if (!foundEq)
        return res.status(404).send({
          message: `Can't process payment. Equipment not found`,
          oc: null
        });
      const foundSucursal = await sucursal
        .findOne({ code: foundEq.sucursal })
        .exec();
      const foundDescuentos = await descuento
        .find(
          {
            sucursales: foundSucursal.code,
            iDate: { $lt: Date.now() },
            eDate: { $gt: Date.now() },
            active: true
          },
        ...
        .exec();
      const descuentoEqs = foundDescuentos
        .filter(desc => {
        ...
        .reduce((descFinal, desc) => {
          ...
          if (desc.descuento[foundEq.type] > descFinal.descuento[foundEq.type])
            return desc;
          return descFinal;
        }, null);
      if (!foundSucursal.precio[foundEq.type])
        return res
          .status(500)
          .send({ message: "error with equipment price", oc: null });
      const pagoEq = {
    ...
    if (!eqs.length)
      return res
        .status(400)
        .send({ message: `Can't process payment. No equipments`, oc: null });
    const price = eqs.reduce(
      (amount, eq, i) =>
        amount + eq.price + eq.discount ? eq.discount.amount : 0,
      0
    );
